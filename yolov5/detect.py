...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            fname = Path(p).name
            save_path = os.path.join(Path(out), fname.split('.')[0] + '-det.' + fname.split('.')[-1])
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    if save_txt:  # Write to file
                        ...
                        with open(save_path[:save_path.rfind('-det')] + '.txt', 'a') as file:
                            file.write(('%g ' * 6 + '\n') % (cls, frame, *xywh))  # label format
    ...
    print('Detection: (%.3fs)' % (time.time() - t0))
