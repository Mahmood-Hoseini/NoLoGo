...
def load_bbox_txt(fname, img_w, img_h) :
    bboxes = []
    frames = []
    with open(fname, 'r') as fd :
        while True:
            ...
            w = int(float(lst[3])*img_w)
            h = int(float(lst[4])*img_h)
            x = int(float(lst[1])*img_w) - w//2
            y = int(float(lst[2])*img_h) - h//2
            bboxes.append([x, y, w, h])
            frames.append(int(lst[0]))
    return bboxes, frames


def create_mask(bboxes, img_w, img_h) :
    mask = torch.zeros((img_h, img_w, 1))
    num_boxes = len(bboxes)
    for ii in range(num_boxes):
        bbox = bboxes[ii]
        mask[bbox[1]:bbox[1]+bbox[3], bbox[0]:bbox[0]+bbox[2], 0] = 1.
    return mask
